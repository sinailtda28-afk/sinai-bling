...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL(`/?error=${encodeURIComponent(searchParams.get("error_description") || error)}`, request.url)
    );
  }

  if (!code || !state) {
    return NextResponse.redirect(new URL("/?error=Codigo_de_autorizacao_ausente", request.url));
  }

  const savedState = request.cookies.get("bling_oauth_state")?.value;
  if (!savedState || savedState !== state) {
    return NextResponse.redirect(new URL("/?error=Estado_invalido_possivel_CSRF", request.url));
  }
